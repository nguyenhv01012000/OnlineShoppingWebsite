...
export default function DashboardMain() {
    ...
    useEffect(() => {
        ...
        axios.get(`http://103.237.145.107:4141/users/list`)
            .then(res => {
                setUser(res.data)
            }
            )
        ...
        axios.get(`http://103.237.145.107:4141/order`)
            .then(res => {
                ...
                if (lastMonthOrder?.length == 0) {
                    setOrderMonthPercent({
                        percent: 0,
                        isDecrease: true
                    })
                }
                else if (currentOrder.length >= lastMonthOrder.length) {
                    setOrderMonthPercent({
                ...
                if (lastCurrentTotalSale == 0) {
                    setSaleMonthPercent({
                        percent: 0,
                        isDecrease: true
                    })
                }
                else if (currentTotalSale >= lastCurrentTotalSale) {
                    setSaleMonthPercent({
                ...
                if (lastCurrentTotalIncome == 0) {
                    setIncomeMonthPercent({
                        percent: 0,
                        isDecrease: true
                    })
                }
                else if (currentTotalIncome >= lastCurrentTotalIncome) {
                    setIncomeMonthPercent({
