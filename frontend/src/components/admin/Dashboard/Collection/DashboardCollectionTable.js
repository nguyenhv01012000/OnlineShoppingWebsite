...
export default function DashboardCollectionTable(props) {
    ...
    useEffect(()=>{
        axios.get(`http://103.237.145.107:4141/collection`)
            .then(res => {
    ...
    const deleteOnClick = (event) => {
        axios.post(`http://103.237.145.107:4141/collection/delete/:${event.target.id}`, {
            id: event.target.id
        })
    ...
    const searchOnChange = (event) => {
        ...
        for (let i in constCollection) {
            if ((constCollection[i].collectionName).toLowerCase().includes(searchInput.toLowerCase())) {
