...
export default function DashboardProductEdit(props) {
    ...
    useEffect(()=> { 
        if (product) {
            ...
            axios.get(`http://14.225.238.137:4141/category`)
                .then(res => {
                    setCate(res.data)
                }
            )
    ...
    const onSubmit = (event) => {
        ...
        axios.post(`http://14.225.238.137:4141/products/update/${product._id}`, formData, config)
        .then(()=>{
    ...
    const addNewCate = () => {
        axios.post('http://14.225.238.137:4141/category', {
    ...
    const deleteImg = (event) => {
        ...
        items.splice(id, 1)
        setProductImg(items)
        axios.post(`http://14.225.238.137:4141/products/update/${product._id}`, {
            deleteImgId: id
        })
    }
