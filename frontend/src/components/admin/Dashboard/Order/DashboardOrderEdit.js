...
export default function DashboardOrderCreate(props) {
    ...
    useEffect(()=>{
        ...
        if (chooseUser === false) {
            axios.get(`http://103.237.145.107:4141/vietnam`)
                .then(res => {
                    ...
                    if (order) {
                        ...
                        if(typeof order.orderList !== "undefined") {
                            order.orderList.map((item)=>{
                                axios.get(`http://103.237.145.107:4141/products/${item.id}`)
                                    .then(res => {
    ...
    const onSubmit = (event) => {
       ...
       if(order.orderStatus == "Delivering") status="Delivered"
        ...
        axios.post(`http://103.237.145.107:4141/order/update/${order._id}`, {
            orderName: orderName,
                                ...
                                { productList && 
                                    productList.map((item, index) => {
                                        return (
                                            ...
                                            >
                                                <img src={item.productImg && item.productImg[0]} alt=""></img>
